...
internal final class VideoPlayer: SharedRef<AVPlayer>, Hashable, VideoPlayerObserverDelegate {
  ...
  var playbackRate: Float = 1.0 {
    didSet {
      if oldValue != playbackRate {
        self.emit(event: "playbackRateChange", arguments: oldValue, playbackRate)
      }
  ...
  var volume: Float = 1.0 {
    didSet {
      if oldValue != volume {
        ...
        self.emit(event: "volumeChange", arguments: oldVolumeEvent, newVolumeEvent)
      }
  ...
  var isMuted: Bool = false {
    didSet {
      if oldValue != isMuted {
        ...
        self.emit(event: "volumeChange", arguments: oldVolumeEvent.isMuted, newVolumeEvent.isMuted)
      }
  ...
  func onStatusChanged(player: AVPlayer, oldStatus: PlayerStatus?, newStatus: PlayerStatus, error: Exception?) {
    ...
    self.emit(event: "statusChange", arguments: oldStatus?.rawValue, newStatus.rawValue, errorRecord)
    status = newStatus
  }

  func onIsPlayingChanged(player: AVPlayer, oldIsPlaying: Bool?, newIsPlaying: Bool) {
    self.emit(event: "playingChange", arguments: oldIsPlaying, newIsPlaying)
  ...
  func onItemChanged(player: AVPlayer, oldVideoPlayerItem: VideoPlayerItem?, newVideoPlayerItem: VideoPlayerItem?) {
    self.emit(event: "sourceChange", arguments: oldVideoPlayerItem?.videoSource, newVideoPlayerItem?.videoSource)
  }
